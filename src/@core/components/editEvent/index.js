...
const EventEditForm = () => {
  ...
  const UserEditAbleData = useSelector(    (state) => state?.event?.eventEditData);
  console.log('editdata to event',UserEditAbleData)
  ...
  const endDateOnly = endDate ? moment(endDate).format("YYYY-MM-DD") : null;
    ...
    try {
      const response = await axios.put(
        `${BASE_URL}/event/newevents/${UserEditAbleData.event_id}/`,
        formDataToSend,
        {
          headers: {
            Authorization: `Bearer ${token}`,
